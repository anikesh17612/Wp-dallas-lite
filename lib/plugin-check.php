...
function dallas_lite_plugin_notice() {
    $user_id = get_current_user_id();
    if ( !get_user_meta( $user_id, 'dallas_lite_notice_dismissed' ) ) {
...
add_action( 'admin_notices', 'dallas_lite_plugin_notice' );

function dallas_lite_notice_dismissed() {
    $user_id = get_current_user_id();
    if ( isset( $_GET['tgmpa-dismiss'] ) )
        add_user_meta( $user_id, 'dallas_lite_notice_dismissed', 'true', true );
}
add_action( 'admin_init', 'dallas_lite_notice_dismissed' );
