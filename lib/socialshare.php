 ...
 * @package Dallas Lite
 */
         function dallas_lite_social_share_icons()
        	{
        	...
        	$twitterURL = esc_url( 'https://twitter.com/intent/tweet?text=' . $title . '&amp;url=' . $url . '&amp;' );
        	$postThumbnail = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID) , 'full');
        	$pinterestURL = esc_url( 'https://pinterest.com/pin/create/button/?url=' . $url . '&amp;media=' . $postThumbnail[0] . '&amp;description=' . $title );
        	$deliciousURL = esc_url( 'https://delicious.com/save?v=' . $url . '&title=' . $title );

          if (esc_attr( get_theme_mod( 'facebookshare' ) ) == 1 )
        		{
        	...
        	if (esc_attr( get_theme_mod('twittershare') ) == 1 || esc_attr( get_theme_mod( 'twittershare' ) ) == "")
        		{
        		$html = $html . "<div class='twitter'><a target='_blank' href='" . $twitterURL . "' ><i class='fa fa-twitter' aria-hidden='true'></i></a></div>";
        		}
        	if (esc_attr( get_theme_mod('linkedinshare') ) == 1 || esc_attr( get_theme_mod( 'linkedinshare' ) ) == "")
        		{
        	...
        	if (esc_attr( get_theme_mod( 'redditshare' ) ) == 1 || esc_attr( get_theme_mod( 'redditshare' ) ) == "")
        		{
        	...
        	if (esc_attr( get_theme_mod( 'emailshare' ) ) == 1 || esc_attr( get_theme_mod( 'emailshare' ) ) == "")
        		{
        	...
        	if (esc_attr( get_theme_mod( 'googleplusshare' ) ) == 1 || esc_attr( get_theme_mod( 'googleplusshare' ) ) == "")
        		{
        		$html = $html . "<div class='delicious'><a target='_blank' href='" . $deliciousURL . "'><i class='fa fa-delicious' aria-hidden='true'></i></a></div>";
        		}
        	if (esc_attr( get_theme_mod( 'pinterestshare' ) ) == 1 || esc_attr( get_theme_mod( 'pinterestshare' ) ) == "")
        		{
        		$html = $html . "<div class='pinterest'><a target='_blank' href='" . $pinterestURL . "'><i class='fa fa-pinterest' aria-hidden='true'></i></a></div>";
        		}
        	if (esc_attr( get_theme_mod( 'deliciousshare' ) ) == 1 || esc_attr( get_theme_mod( 'deliciousshare' ) ) == "")
        		{
        	...
        	if (esc_attr( get_theme_mod( 'stumbleuponshare' ) ) == 1 || esc_attr( get_theme_mod( 'stumbleuponshare' ) ) == "")
        		{
